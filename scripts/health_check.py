...
def check_scheduler() -> dict:
    """Check if scheduler process is running (no duplicates)."""
    try:
        import subprocess
        ...
        if count == 0:
            return {"status": "critical", "count": 0, "detail": "No scheduler running"}
        elif count <= 2:
            # Windows/MSYS spawns 2 PIDs per `python` command (venv + uv). 1-2 PIDs = 1 logical scheduler.
            return {"status": "healthy", "count": count, "detail": "Single scheduler running"}
        else:
