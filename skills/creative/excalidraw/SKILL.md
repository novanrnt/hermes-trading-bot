...
## Alternative: Dark SVG Architecture Diagrams (HTML Format)

For professional, dark-themed infrastructure/cloud architecture diagrams (NOT hand-drawn), use standalone HTML files with inline SVG instead of Excalidraw JSON. These produce a polished, grid-backed aesthetic suitable for software system architecture, cloud infrastructure, and microservice topology.
...
Single self-contained `.html` file. All CSS and SVG inline. No JavaScript. Components are rounded rectangles with double-rect masking for proper opacity handling. Arrows must be drawn BEFORE component boxes in the SVG (z-order: grid pattern → arrows → components).

### Structure

1. **Header** — title with pulsing dot indicator and subtitle
2. **Main SVG** — diagram within a rounded card on slate-950 background with 40px grid
3. **Summary Cards** — three info cards below the diagram
4. **Footer** — minimal metadata

Font: JetBrains Mono from Google Fonts. Background: `#020617` (slate-950) with `<pattern>` grid overlay.
